...
complex<double> b0(vector<vector<complex<double>>>& f, int i) {
    ...
    for (int n = 1; n <= nmax; n++) {
        bi += sqrt(1.0 * n) * conj(f[i][n - 1]) * f[i][n];
    }
    return bi;
}
...
complex<double> b1(vector<vector<complex<double>>>& f, int i, vector<double>& J, double U) {
    ...
    for (int n = 0; n < nmax; n++) {
        for (int m = 1; m <= nmax; m++) {
            if (n != m - 1) {
                bi += -J[i] * g(n, m) / eps(U, n, m) * sqrt(1.0 * n + 1) * conj(f[j2][m - 1]) * f[j2][m] * (conj(f[i][n + 1]) * f[i][n + 1] - conj(f[i][n]) * f[i][n]);
                bi += -J[j1] * g(n, m) / eps(U, n, m) * sqrt(1.0 * n + 1) * conj(f[j1][m - 1]) * f[j1][m] * (conj(f[i][n + 1]) * f[i][n + 1] - conj(f[i][n]) * f[i][n]);

                if (m < nmax) {
                    bi += -J[i] * g(n, m) / eps(U, n, m) * sqrt(1.0 * m + 1) * conj(f[j2][n + 1]) * f[j2][n] * conj(f[i][m - 1]) * f[i][m + 1];
                    bi += -J[j1] * g(n, m) / eps(U, n, m) * sqrt(1.0 * m + 1) * conj(f[j1][n + 1]) * f[j1][n] * conj(f[i][m - 1]) * f[i][m + 1];
                }
                if (m > 1) {
                    bi += J[i] * g(n, m) / eps(U, n, m) * sqrt(1.0 * m - 1) * conj(f[j2][n + 1]) * f[j2][n] * conj(f[i][m - 2]) * f[i][m];
                    bi += J[j1] * g(n, m) / eps(U, n, m) * sqrt(1.0 * m - 1) * conj(f[j1][n + 1]) * f[j1][n] * conj(f[i][m - 2]) * f[i][m];
                }
            }
        }
    }
